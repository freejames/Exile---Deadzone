...
while {ExileClientConstructionResult isEqualTo 0} do
{
	...
	if !(ExileClientConstructionLock) then
	{
		...
		if (!ExileClientConstructionVectorMode) then {
			...
			//[ExileClientConstructionObject, ExileClientConstructionPitch, ExileClientConstructionBank] call BIS_fnc_setPitchBank;
			[ExileClientConstructionObject,[ExileClientConstructionPitch,ExileClientConstructionBank,_rotation]] call GM_SetPitchBankYaw;
		}
		else
		{
			_rotation = (ExileClientConstructionRotation + (getDir ExileClientConstructionCurrentSnapToObject) + 360) % 360;
